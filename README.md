...
**Integrating with LOAM**: The original implementation of the [Lidar Odometry and Mapping (LOAM)](http://wiki.ros.org/loam_velodyne) algorithm uses the camera coordinate convention (x-left, y-up, z-front). This repository uses the standard ground robot convention (x-front, y-left, z-up). If using the original version of LOAM for state estimation, comment out
