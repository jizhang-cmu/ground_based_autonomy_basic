The repository contains code for collision avoidance to use with ground robots. In an autonomous navigation system, this repository is often in the middle, above the state estimation module and below the high-level planning module. To help users start easily, the repository includes a vehicle simulator and does not need to run onboard a physical robot. The code operates in two modes: *autonomy mode* takes waypoints as the input and navigates towards the waypoints while avoiding obstacles along the way; *smart joystick mode* takes commands from an operator through a joystick controller and avoids obstacles that the robot approaches. The *smart joystick mode* is often useful in debugging the navigation system and is highly recommended. This is because the operator can command the robot to move in any direction for testing purposes.
